...
BASE_DIR=os.getcwd()
INPUT_FILE=os.path.join(BASE_DIR,"global_water_consumption_2000_2025.csv")
...
target=[c for c in numeric if c!=year_col][0]
...
ax.set_title("Water Consumption Anomaly Detection")
ax.set_xlabel("Year")
...
ax.set_title("Consumption Structural Clustering")
ax.set_xlabel("Principal Component 1")
ax.set_ylabel("Principal Component 2")
...
ax.set_title("Water Consumption Frequency Spectrum")
...
rolling_mean=df[target].rolling(5).mean()
rolling_std=df[target].rolling(5).std()

fig,ax=plt.subplots(figsize=(14,8))
ax.plot(df["date"],df[target],alpha=0.3)
ax.plot(df["date"],rolling_mean,color=ACCENT,linewidth=3)
ax.set_title("Rolling Consumption Mean")
ax.set_xlabel("Year")
ax.set_ylabel("Consumption")
...
ax.set_title("Rolling Consumption Volatility")
ax.set_xlabel("Year")
...
if "region" in df.columns:
    region_year=df.groupby(["region",year_col])[target].mean().unstack()
    fig,ax=plt.subplots(figsize=(14,8))
    sns.heatmap(region_year,cmap="viridis",ax=ax)
    ax.set_title("Regional Consumption Heatmap")
    fig.savefig(os.path.join(CHART_DIR,"region_heatmap.png"),dpi=300,bbox_inches="tight")
    plt.close()
...
ax.set_title("Cumulative Consumption Growth")
ax.set_xlabel("Year")
ax.set_ylabel("Growth")
fig.savefig(os.path.join(CHART_DIR,"cumulative_growth.png"),dpi=300,bbox_inches="tight")
plt.close()
...
ax.set_title("Feature Importance for Consumption")
...
id_col=[c for c in df.columns if c not in numeric and c not in ["date","month","week","cluster"]]
id_col=id_col[0] if id_col else df.columns[0]
...
for i in range(len(df)):
    ...
    for j,score in sim_scores:
        recommendations.append({
            "source_entity":df.iloc[i][id_col],
            "recommended_entity":df.iloc[j][id_col],
            "similarity":score
        })

rec_df=pd.DataFrame(recommendations)
rec_df.to_csv(os.path.join(REC_DIR,"water_consumption_recommendations.csv"),index=False)
...
doc=SimpleDocTemplate(os.path.join(OUTPUT_DIR,"Global_Water_Consumption_Intelligence_Report.pdf"),leftMargin=72,rightMargin=72,topMargin=72,bottomMargin=72)

elements=[]
elements.append(Paragraph("Global Water Consumption Intelligence Report",title_style))

summary=f"""
Executive Summary<br/><br/>
Dataset Size: {len(df)} records<br/>
Years Covered: {df[year_col].min()} to {df[year_col].max()}<br/>
Cluster Quality Score: {round(sil,3)}<br/>
Execution Time: {execution} seconds<br/><br/>
This report provides structural intelligence, anomaly detection, and consumption pattern modeling across regions and time.
"""
...
for chart in charts:
    ...
    elements.append(Paragraph("This chart presents structural intelligence extracted from global water consumption patterns.",body_style))
    elements.append(PageBreak())
